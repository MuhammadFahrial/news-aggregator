const API_KEY = "9ddab756845e4422a9d321f36b9b351a";
...
const fetchNews = (isSearching) => {
  ...
  if (isSearching) {
    ...
    url = `
    https://newsapi.org/v2/everything?q=${keyword}&apiKey=${API_KEY}&page=${currentPage}`;
  } else {
    const category =
    ...
    .then((data) => {
      ...
      articlesWithImage.forEach((article) => {
        ...
        <div class='newsItem'>
            ...
            <div class='info'>
            <h5>${article.source.name}</h5><span>|</span><h5>${article.publishedAt}</h5></div>
            <h2>${article.title}</h2><p>${article.description}</p><a href='${article.url}' target='_blank'>Read More</a></div>
            </div>
            `;
